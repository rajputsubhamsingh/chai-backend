...
const loginUser = asyncHandler(async (req, res) => {
    ...
    // if(!(username || email)) {  // wala condition bhi same kam karega bilkul niche wale condition ke tarah 
    if(!username && !email) {
    ...
    // httpOnly || secure jb false hoga to hmre cookies ho koi frontend se modified kar dega
    // but jb true hoga to only server se modified hoga 
...
const logoutUser = asyncHandler(async (req, res) => {
    await User.findByIdAndUpdate(
        ...
        {
            new: true   // new jo hai ab update value ko return kar dega
        }
    );
...
const updateAccountDetails = asyncHandler(async (req, res) => {
    ...
    const user = await User.findByIdAndUpdate(
        ...
        {
            $set: {
                // fullName: fullName  -> niche single fullName hai joki es6 ka syntax hai
                fullName,
                email: email
            }
        },
